...
* `Town.last_built_turn` for construction availability in pure/design rules.
  The current v1.1 canister records the field after generic builds but does
  not enforce one generic building per town per turn; it enforces ownership,
  resources, prerequisites, and duplicate-building rejection.
...
V1.1 UI note: this section is design background. Only render controls for
mechanics exposed by Part 2 endpoints or explicit disabled legal-action
metadata.
...
3. In v1.1, the attacker automatically captures the town after winning the
   town battle. Do not show occupy, raze, or pacify choices; those choices are
   V2.
...
Current v1.1 canister behavior: generic town build preview/submit prevents
duplicate building slugs and missing prerequisites, but does not enforce this
per-turn construction cooldown for all generic buildings.
...
Current v1.1 canister behavior uses the fixed rates above. Marketplace-owner
rate improvements are future economy expansion unless promoted with tests.
...
building slug is not already built in that town
```

`Town.last_built_turn` is still written for projection/design compatibility,
but the current v1.1 canister generic build path does not reject a second
different generic building solely because it is the same turn.
...
If now >= GameSession.turn_deadline_at:
  advance at most one turn in the current v1.1 canister
...
```text
One building per town per turn: design-only cooldown; current generic canister builds do not enforce it
Unit growth every 7 turns
Base/capitol town income: deferred content economy, not active v1.1 canister income
```
...
> Every match has a 60-second maximum turn window. Players submit replaceable movement intents and other commands during that window. Players may also end their map turn early. Ending turn is a readiness signal, not a lock: an ended player may still submit map commands while the same turn remains open. The turn closes when either every active map participant has ended the current turn or the 60-second deadline expires. When the turn closes, movement intents resolve in deterministic turn-end order. The turn advances through a canister-owned update context: the final `end_turn` call, an IC timer callback, a zero-delay continuation, or a public sync/recovery update call. Normal clients may call `sync_session_turn` when render/query metadata reports `sync_required`, and may call `sync_battle` to make battle deadlines, timeout recovery, or aftermath visible promptly. They do not call a generic `process_next_turn` endpoint. Query calls may return time metadata, but must not return speculative movement, battle, victory, resource, visibility, or event results. Backend-work pressure appears through command responses, command status, sync outcomes, and battle state. Anyone who did nothing simply skips that turn.
...
```text
Battle action timeout target: 30 seconds per active stack.
The canister allows a valid active-stack action until action_deadline_at +
15_000ms if it wins the race against timeout processing.
...
If now >= action_deadline_at and the submit grace has also elapsed, a
deterministic system auto-defend command wins unless a valid player action was
already applied.
...
Public API payloads expose ids as `String`/Candid `Text`; generated/internal
code may still use `Id<T>` wrappers behind the service boundary. Entity
relation fields store primitive keys.
...
Multi-window catch-up is deferred until it has bounded-loop/jobs coverage and
tests.
```
...
jobs. `sync_battle` is the public battle sync/recovery boundary over the same
...
`MoveCoord` is `{ x: u16, y: u16 }` on the public wire contract.
...
4. V1 clients submit while the render-time DTO reports the current turn as
   ...
   clients call `sync_session_turn` when metadata reports `sync_required`, then
   poll/refresh persisted state. Once turn resolution is accepted for the old
   turn, `submit_move_intent` fails stale/turn-expired and the client
   refreshes.
...
9. Current canister validation allows static undiscovered terrain; dynamic
   hidden objects, occupants, owners, battle details, and events remain hidden
   unless visibility rules allow them.
10. Path length <= 64 steps in v1. Pure rules track an 8-chunk touched cap;
    the current canister reports `chunks_touched` but does not reject by chunk
    count.
...
Public APIs return `ApiEventView` DTOs, never raw `GameEvent.payload_json`.
...
Every pseudo-random outcome records domain_key and roll_index in the command
result or public event payload for audit.
```
...
The exhaustive current endpoint inventory is `REQUIRED_GAME_ENDPOINTS` in
`canisters/degens/src/contract.rs`, mirrored by `docs/canister-endpoints.md`.
The list below documents the UI-facing shape for the active v1.1 client
contract; generated Candid remains authoritative for exact wire types.
Public Candid ids are `String`/`Text`, not internal `Id<T>` wrappers.
Movement paths use `MoveCoord { x: u16, y: u16 }`.

Recommended gameplay update calls:

```rust
register_player(username: Option<String>, display_name: Option<String>, client_nonce: String)

create_session(name: String, ruleset_id: String, seed: u64, client_nonce: String)

join_session(session_id: String, faction_id: String, client_nonce: String)

start_session(session_id: String, client_nonce: String)

end_turn(session_id: String, client_nonce: String)

end_battle_turn(session_id: String, battle_id: String, client_nonce: String)
...
mark_ready(session_id: String, client_nonce: String)
```

`mark_ready` is lobby-only. `end_turn` is the active map-turn readiness command.

Public sync/recovery update calls:

```rust
sync_session_turn(session_id: String, client_nonce: String)

sync_battle(session_id: String, battle_id: String, client_nonce: String)
```

Additional required update endpoints cover the implemented checkpoint 22-25
slices:

```rust
select_champion_level_up(session_id, champion_id, skill_key, client_nonce)
learn_champion_spell(session_id, champion_id, spell_slug, client_nonce)
cast_adventure_spell(session_id, champion_id, spell_slug, client_nonce)
hire_tavern_champion(session_id, town_id, offer_key, client_nonce)
submit_market_trade(session_id, from_resource, to_resource, amount_in, client_nonce)
submit_dwelling_recruit(session_id, object_id, unit_slug, quantity, champion_id, client_nonce)
accept_quest(session_id, quest_key, client_nonce)
claim_quest_reward(session_id, quest_key, client_nonce)
sync_objectives(session_id, client_nonce)
sync_world_events(session_id, client_nonce)
sync_advanced_victory(session_id, client_nonce)
sync_world_generation(session_id, client_nonce)
```
...
Champion { champion_id: String, slot_index: Option<u8> }
```

V1.1 canister town recruitment supports both `TownGarrison` and
same-tile `Champion` targets. Champion-target recruitment requires the champion
to be active, owned by the participant, not in battle, and on the same tile as
the owned town. Compatible stacks merge, empty legal slots create a stack, and
no legal slot returns `recruit_target_full`. Invalid champion targets return a
stable failed command response before resource spending, pool decrement, or
stack mutation.

`sync_session_turn` and `sync_battle` are public sync/recovery update calls.
They call the same bounded progression paths used by timers, zero-delay
continuations, awaited self-call continuations, and ordinary gameplay
commands. Normal clients use `sync_session_turn` when render/query metadata
reports `sync_required`, and may use `sync_battle` to make battle deadlines,
timeout recovery, or aftermath handoff visible promptly. They should not be
presented as flavorful gameplay actions. Query calls must not persist turn
advancement, battle timeout, command recovery, or other lazy-state changes.

`get_object_view` intentionally returns `not_visible` for hidden, unknown, or
no-longer-visible subjects so clients cannot infer absence. `get_town_view` and
`get_champion_view` may return `not_found` for invalid or missing ids, and
`not_visible` for existing hidden rows.
...
preview_champion_progression(session_id, champion_id)

get_tavern_offers(session_id, town_id)

preview_hire_champion(session_id, town_id, offer_key)

preview_market_trade(session_id, from_resource, to_resource, amount_in)

get_dwelling_pool(session_id, object_id)

preview_dwelling_recruit(session_id, object_id, unit_slug, quantity, champion_id)

get_objective_progress(session_id)

get_scenario_rules(session_id)

get_world_events(session_id)

preview_quest(session_id, quest_key)

get_skirmish_settings(session_id)

get_procedural_map_state(session_id)

get_naval_routes(session_id)

get_siege_rules(session_id)
```

## API Response Contracts

`GameViewRequest` uses flat per-list cursor and limit fields:
...
`GameView` has separate `map_page_info`, `object_page_info`, and
`event_page_info` fields. A cursor is scoped to exactly one list; never reuse an
object cursor as an event cursor.

Render-time metadata is exposed through `GameView.render_time`:
...
Current canister `sync_required` means `server_now_ms` has reached the
persisted turn deadline. Other backend-work conditions surface through command
errors such as `backend_work_pending`, `battle_processing`, retryable command
status, or incomplete sync outcomes, not as a `GameView` field. The client
should call `sync_session_turn` when metadata reports `sync_required`, may call
`sync_battle` for battle timeout/recovery/aftermath catch-up, then poll,
subscribe to events, or refresh affected views. These sync endpoints are
recovery/progression boundaries, not gameplay action affordances.

Query methods may derive read-only time metadata such as `server_now_ms`,
`turn_started_at_ms`, `turn_duration_ms`, and whether sync is required. Resource,
recruit, movement, visibility, battle, victory, and event values come from
persisted rows or active runtime projections at the current durable/runtime
boundary. Query methods must not simulate or persist movement-intent resolution,
battle timeouts, battle auto-defend, victory finalization, town capture,
resource ledger writes, recruitment growth writes, visibility writes, command
recovery, or event creation. If the turn deadline is reached, render queries
report `sync_required = true` and do not advance positions.
...
CommandResponse {
  ...
  command_type
  actor_principal
  actor_participant_id: Option<Text>
  client_nonce
  payload_hash
  status
  phase
...
`LobbyCommandResponse` uses the same envelope but has no
`actor_participant_id` field and returns `LobbyCommandResult`.
...
`MovementSync` and `BattleSync` are returned by public sync/recovery update
calls. UI clients use `sync_session_turn` when `render_time.sync_required` is
true and may use `sync_battle` to make due battle timeout/recovery/aftermath
state visible promptly.

`ChangedSubject` includes `subject_kind`, `subject_id_text`, and `operation`.

`ApiError` includes `code`, `message`, `details_json`, and `retryable`.

Common public error/status codes include, but are not limited to:
...
battle_not_active
not_active_stack
battle_stack_not_found
battle_stack_not_active
...
`already_built_this_turn` and movement `too_many_chunks` are pure/design error
codes; current canister v1.1 generic town builds use `already_built` for
duplicate building slugs and do not enforce a hard per-turn construction
cooldown, while movement reports touched chunks without rejecting by chunk
count.

This list is illustrative; generated Candid plus the service code are
authoritative for exact route-level errors. Current incomplete-sync signals are
typed outcomes such as `MovementSyncOutcome.budget_exhausted`,
`movement_sync_incomplete` events, and `BattleSyncOutcome.battle_sync_incomplete`.
`system_job_pending` and a generic `sync_incomplete` error name are historical
planning labels, not current canister error codes. Clients should poll command
status and refresh composed render state when they see an incomplete sync
boundary.

The canister render contract is composed from bounded render endpoints, not raw
IcyDB rows. `get_game_view` is a lightweight gameplay metadata shell on the
canister:
...
GameView {
  ...
  action_affordances: ActionAffordance[]
  omitted_fields: Text[]
}
```

The canister currently omits heavy map/object/champion/town collections from
`get_game_view`, lists `map_chunks`, `objects`, `champions`, and `towns` in
`omitted_fields`, and returns empty omitted collections with `has_more = false`
and `next_cursor = None`. Battle detail is also not in the shell response:
`battle` and `battle_summary` are `None`, but they are not listed in
`omitted_fields`.
The canister backend exposes render-ready state through dedicated bounded
endpoints
...
```text
SessionSummary:
  session_id, state, participant_ids, current_turn

ParticipantSummary:
  participant_id, player_id, faction_slug, slot_index, status, ready,
  resources
...
ChampionView:
  ...
  x, y, effective_movement, movement_max, mana, mana_max, skill_points,
  skill_keys, spell_slugs, vision_radius, strength_label,
  army_stacks: ChampionArmyStackRecord[], artifacts: ArtifactView[], redacted

TownView:
  ApiTownView { town, buildings[], recruit_pools[], garrison_stacks[] }

ChampionArmyStackRecord:
  stack_id, session_id, champion_id, unit_slug, slot_index, quantity, front_hp,
  status, last_command_id

BattleSummary:
  battle_id, state, battle_type, current_round, active_stack_id,
  active_participant_id

ActionAffordance:
  action, enabled, target_id, disabled_reason

PageInfo:
  PageInfo { next_cursor, has_more, limit }
  EventPageInfo { next_event_seq, has_more, limit }
```
...
`details_json` is the redacted, render-oriented details payload for the subject
kind. Known world-object list rows may expose live owner/state from the current
projection; town/garrison and enemy champion details stay redacted unless the
caller is allowed to see the underlying subject view.
```
...
Readiness is represented through command responses, events, active
stack/participant, and timeout metadata; the current `BattleView` does not
include ready participant arrays, per-caller round-ready fields, or deployment
zones.

`BattleCoord` is `{ x: u8, y: u8 }`.

`BattleObstacleView` includes `battle_obstacle_id`, `obstacle_type`,
`battle_x`, `battle_y`, `width`, `height`, `hp`, and `state`.

`LegalBattleAction` includes `action`, optional `ability_key`, `enabled`,
`disabled_reason`, `targets`, `path`, and optional `damage_preview`.
`BattleStackView` includes unit metadata needed for rendering and input:
`battle_stack_id`, `unit_id`, `side`, `owner_participant_id`, `battle_x`,
`battle_y`, `quantity`, `front_hp`, `shots_remaining`, `champion_might`,
`champion_guard`, `attack`, `defense`, `damage_min`, `damage_max`, `max_hp`,
`speed`, `initiative`, `ranged`, `flying`, `acted_round`, `waited_round`,
`defended_round`, `status`, and `status_keys`.

Public API event DTO:
...
`ApiEventView.payload` is redacted for the caller. Public APIs never return raw `GameEvent.payload_json`.
...
Legal `action` strings are `Move`, `MeleeAttack`, `RangedAttack`,
`CastAbility`, `Defend`, and `Wait`. `Move` uses `destination`; attack actions
use `target_stack_id`; `CastAbility` uses `ability_key` and, when needed,
`target_stack_id` or `destination`. `CastAbility` is active for learned v1.1
battle spells. `Retreat` and `Surrender` are deferred action names that may
appear only as disabled legal-action metadata. Clients must not submit them; if
they are submitted, current validation returns a failed command response such
as `battle_action_not_supported`.

Pagination contract:

```text
Paged public endpoints:
  get_visible_map_chunks(session_id, viewport, cursor, limit)
  get_visible_objects(session_id, viewport, cursor, limit)
  get_events_after(session_id, audience_key, events_after_seq, limit)
  get_match_history(cursor, limit)

get_game_view has per-list cursors and limits in GameViewRequest.
Default list limit: 50
Max list limit: 200
Max chunk limit: 9
Max object limit: 200
Max events in get_game_view: 50
Max viewport tiles: MAX_VIEWPORT_TILES
Max active battles per session: 2
Ordering must include a stable tie-breaker such as id or event_seq.
MapChunkPage exposes `chunks`, `next_cursor`, and `has_more`; ObjectViewPage
exposes `objects`, `next_cursor`, and `has_more`.
GameView list metadata uses PageInfo/EventPageInfo.
Match history uses PageInfo.
Some canister cursors are numeric offsets over the current sorted projection;
they are live-state cursors, not snapshot-isolated.
Events order by session_id + event_seq.
Event cursors are numeric events_after_seq values; responses include next_event_seq.
Cursor pagination is live-state, not snapshot-isolated; clients must tolerate row changes between pages.
Viewport tile overflow rejects with viewport_too_large.
Chunk page overflow rejects with viewport_chunk_limit_exceeded.
```
...
If `GameView.render_time.sync_required = true`, the client calls
`sync_session_turn` with a fresh typed nonce, then waits/polls, subscribes to
events, or refreshes affected views.
If battle state is active or resolved and timeout/recovery/aftermath work may
be due, the client may call `sync_battle` with a fresh typed nonce and then
refresh battle, session, map, champion, town, event, and history views.
If a command returns `backend_work_pending`, `battle_processing`,
`recovery_budget_exhausted`, an incomplete sync outcome, or status = `Applying`
with `retryable = true`, the client polls `get_command_status` by command id or
`get_command_status_by_nonce(session_id, command_type, client_nonce)` by the
explicit command type and then refreshes composed render state.
After any applied command, the client applies returned `ApiEventView` and
`changed_subjects`, then refreshes composed render state using the latest
per-list cursors.
```
...
Query calls return persisted/runtime projections at the current boundary plus
time metadata such as `render_time.sync_required`.
...
`Town.last_built_turn` is updated after a successful generic build, but current
canister validation does not enforce it as a hard per-turn cooldown.
...
Read APIs may return effective resources without writing `GameParticipant`, but update commands must materialize owed active income with ledger rows before spending or rewarding resources. In v1.1 the active recurring income source is captured mine world objects. Town/building income effects such as `town_income_gold_250` are content/deferred unless promoted; pickups, rewards, costs, tavern hiring, market trades, and dwelling recruitment remain active economy paths.

Income ownership cutover:

```text
Before any command changes an active income source, owner, or income rate, materialize owed income for affected participants through current_turn using the old ownership/rate.
Income from a source accrues only from max(participant.last_income_turn, source.captured_turn, source.income_started_turn).
Building construction materializes active owner income before adding the building. Current canister builds do not activate town-building income rates.
Capture materializes active old-owner and new-owner income before changing owner_participant_id where an active source is changing owner.
...
AI driver shape if promoted to public endpoints later:

```rust
run_ai_for_session(session_id: String, max_actors: u16, client_nonce: String)
...
These AI driver methods are not part of the current required gameplay endpoint
inventory. Current v1.1 canister behavior keeps neutral/battle automation
inside battle, timer, and sync flows. If public AI driver endpoints are
promoted later, they must use the same IcyDB command system:
...
Movement validation stops at the first supported interaction or blocker.
submit_move_intent path cap: 64 steps. Pure rules retain an 8-chunk touched
cap; current canister validation reports touched chunks but does not reject by
chunk count.
```
...
Turn-final movement resolution is sliced. If any movement cap would be exceeded, resolve deterministically up to the cap, persist the partial resolution cursor in the turn-resolution system command/effects, and schedule a zero-delay continuation job. Public `sync_session_turn` may drive the same continuation for client recovery/progression when metadata reports `sync_required`.

Battle timeout processing is sliced. No update may auto-resolve an entire stale battle unless the battle fits within the timeout caps; otherwise persist cursor state and schedule a zero-delay continuation job. Public `sync_battle` may drive the same continuation for battle timeout/recovery/aftermath catch-up.
...
Setup phases:
  ...
  seed_participants (idempotent compatibility phase; create/join/ready already created participant rows)
  seed_towns
  seed_champions
  seed_map_chunks
  seed_occupancy
...
stalemate target: if max_turns reached, score by towns owned, then mines owned, then army combat power, then seeded tie-break
```
...
Current canister limitation: max-turn scenario rules can report
`max_turn_reached`, but full stalemate finalization by mines, army power, and a
distinct `max_turn_score` finish reason is not the public UI contract. The UI
should treat `GameSession.state`, `winner_participant_id`, `finish_reason`,
victory events, and match history as authoritative.
...
battle timeout: active stack auto-defends after the 30 second target plus the
15 second submit grace if no valid player action wins first
...
Before changing town owner, materialize owed active income for old and new
owners using the pre-capture ownership/rate. Town capture income cutover is
non-mechanical for town-building income in v1.1 unless town income is promoted.
...
```text
Town/building income effects, captured-town unrest mechanics, pacification,
recruit-pool halving, and desperation income are deferred to v2. In v1.1,
...
First-playable UI flow:

```text
register_player -> create_session -> join_session -> mark_ready
host start_session once
poll get_setup_progress, get_session, and/or events until state == "active"

During play:
  compose get_game_view with visible map/object/champion/town/battle endpoints
  use preview_* before submit_*
  call end_turn for early map-turn closure
  call sync_session_turn when render_time.sync_required is true
  call sync_battle when battle timeout/recovery/aftermath work is due
  after applied commands, consume changed_subjects/events and refresh affected views

Results:
  GameSession.state, winner_participant_id, finish_reason,
  victory_finalized events, and match history
```
