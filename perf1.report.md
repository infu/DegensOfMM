...
| metric | before | latest recorded | change |
| --- | ---: | ---: | ---: |
...
After that, the bottleneck moved away from battle submits. The main remaining hot path became movement/session turn processing. The movement work has improved Gate J, but it has not hit the next target yet:

| metric | movement baseline | latest recorded | change |
| --- | ---: | ---: | ---: |
...
| Current partial work | `git status --short` | `movement.rs` modified, `idea.md` untracked |
...
| Worktree | There is an uncommitted partial edit removing `movement_intent` command effects from `movement.rs`; it is not benchmarked and is not included in the numbers above. `idea.md` is also untracked. |
