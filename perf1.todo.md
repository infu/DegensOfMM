...
- [ ] Gate 5H: add `flush_barrier(reason)` for `TurnAdvance`, `BattleHandoff`, `Upgrade`, `RuntimeEviction`, and `StrongRead`, with idempotent durable projection of commands/events/intents/snapshots/champions/occupancy/visibility/objects/resources/jobs.
  - [ ] Include runtime lobby command receipts in the flush/upgrade/history strategy, or explicitly bound/evict them with a durable replay/status alternative.
...
- [ ] Add durable flush/upgrade snapshot coverage for runtime town command/event/resource/building/pool/garrison data. Active gameplay and Gate J are fast, but recovery/history still need an explicit bounded flush barrier instead of immediate row writes.
- [ ] Broaden `TownProjection` beyond the first-playable alias fast path: tavern offers/growth, all town public-id aliases, cache-miss adoption, and any non-Gate-J town command variants.
