...
- [ ] Gate 5E: make turn advance runtime-owned: next runtime turn, deadline, ready state, and job/deadline hints live in heap; durable `GameSession`, `GameParticipant`, and `SystemJob` rows flush/checkpoint later. Target `sync_session_turn` `0.6B-0.9B`.
  - [x] Gate 5E.1: skip the pre-deadline scheduled-job scan for map-turn commands only when `SessionTurnRuntime` proves the same active session/turn/deadline, no closing state, and no ready participants. `end_turn` still performs the durable duplicate-ready check. Verified with `cargo fmt --check`, normal/benchmark canister checks, and native PocketIC stale-turn regression `pocket_ic_end_turn_closes_turn_and_blocks_stale_actions` in `188.77s`; focused Gate J measurement is deferred to the next batched run. Expected submit/town command repo-op reduction: remove the remaining safe pre-deadline `system_jobs.by_session_status_due` scan when runtime is authoritative.
