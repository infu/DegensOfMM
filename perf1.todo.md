...
- [ ] Gate 5B: make fresh `submit_move_intent` a pure runtime command: no `sessions.load_session`, no `champions.load_champion`, no `SystemJob` guard scan, no durable `MovementIntent`, no durable event/session update, and no stable occupancy/blocker reads. Target `0.3B-0.6B`.
  ...
  - [ ] Gate 5B.5: remove the remaining late `submit_move_intent` durable job scan after partial movement sync. Current Gate J `20260520-seed-content-cache-gate-j` still has seq 61 at `0.7063B` with `system_jobs.by_session_status_due` x2 while the other two move submits are `0.0088B` and `0.0003B`. Investigate why the relaxed active-runtime proof does not cover that post-pickup/post-town move context, then drive all submit-move calls into the `0.3B-0.6B` band or lower.
