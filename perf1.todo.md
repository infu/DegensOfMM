...
- [x] Skip durable event-key absence reads for fresh manual `sync_session_turn` movement/sync events while keeping the old idempotent `append_public_event` path for recovered pending/applying commands. Verified with `cargo fmt --check`, `cargo check -p domm-degens-canister --features benchmark`, `cargo check -p domm-degens-canister`, `cargo test -p domm-degens-canister session_turn_runtime -- --nocapture`, and native PocketIC `pocket_ic_timer_jobs_deadline_resolves_multistep_movement_without_sync` in `189.23s`. Measurement pending the next focused Gate J run; expected effect is lower `events.by_session_event_key` calls on fresh sync continuations.
